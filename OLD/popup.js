...
(function () {
  ...
  function refreshDailyCards() {
    ...
    window.SFLDaily.KEYS.forEach(diff => {
      ...
      if (!rec || (!rec.solved && !rec.gaveUp)) {
        ...
        timeEl.textContent = rec.time ? formatTime(rec.time) : '';
        actionEl.textContent = 'REVIEW';
        actionEl.className = 'daily-card-action review';
  ...
  function setBackMode(active) {
    ...
    if (active) {
      ...
      // Make grid read-only
      document.getElementById('grid').classList.add('grid-readonly');
      ...
      document.getElementById('grid').classList.remove('grid-readonly');
      document.getElementById('checkBtn').disabled = false;
      document.getElementById('resetGridBtn').disabled = false;
    }
  }
      ...
      const observer = new MutationObserver(() => {
        if (resultOverlay.classList.contains('open')) {
          ...
          setTimeout(() => {
            ...
            const gaveUp = (grade === 'F');

            captureDailyCompletionState(solveTime, gaveUp, mistakes, 0, grade);
          }, 100);
        }
      });
